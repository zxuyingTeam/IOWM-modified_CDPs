        ...
        try:
            for e in range(nepochs):
                ...
                if test_acc > self.test_max:
                    ...
                    best_model = utils.get_model(self.model)
                save_data.append([train_loss, 100 * train_acc, valid_loss, 100 * valid_acc, 100 * self.test_max,100 * test_acc])
        ...
        for i_batch in range(0, len(r_len), self.sbatch):
            ...
            def pro_weight(p, x, w, alpha=1.0, cnn=True, stride=1):
                if cnn:
                    ...
                    # w.grad.data = torch.mm(w.grad.data.view(F, -1), torch.t(torch.from_numpy(p).type(dtype))).view_as(w)
                else:
                    r = x
                    k = torch.mm(p, torch.t(r))
                    deltap = torch.mm(k, torch.t(k)) / (alpha + torch.mm(r, k))
