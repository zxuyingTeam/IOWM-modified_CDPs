...
class Appr(object):
    ...
    def __init__(self, model, sbatch=40, lr=0.2, input_size=784, hidden=800, tasks=10):
        self.model = model
        self.sbatch = sbatch
        self.lr = lr
        self.tasks = tasks
    ...
    def _get_optimizer(self, pa_lr=0):
        lr = pa_lr
        lr_owm = pa_lr
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid, data, num_epochs=20, nums=0):
        ...
        all_step = all_data // self.sbatch * nepochs
        # Loop epochs
        try:
            for e in range(nepochs):
                ...
                print('| [{:d}/10], Epoch {:d}/{:d}, | Train: loss={:.3f}, acc={:2.4f}% |'.format(t + 1, e + 1,
                                                                                                 nepochs, train_loss,
                                                                                                 100 * train_acc),
                      end='')
                # # Valid
                ...
                xtest = data[self.tasks]['test']['x'].cuda()
                ytest = data[self.tasks]['test']['y'].cuda()
