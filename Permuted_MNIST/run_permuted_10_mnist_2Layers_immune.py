...
parser.add_argument('--immune_r', type=int, default=15, help="the radius of trust region")
...
alpha = 1.0
# Layer1
w1, force_layer1 = get_layer([28*28, 100], alpha=alpha, args=args)
b1 = get_bias(w1.size(1))
# Layer_out
w2, force_layer2 = get_layer([100, class_num], alpha=alpha, args=args)
...
for task_index in range(Task_num):
    learning_rate = 0.5 + 0.1 * task_index
    ss = np.arange(28*28)
    if task_index > 0:
        np.random.seed(task_index)
        np.random.shuffle(ss)
