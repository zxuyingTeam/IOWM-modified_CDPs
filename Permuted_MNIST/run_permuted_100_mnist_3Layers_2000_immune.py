...
parser.add_argument('--seed_num', type=int, default=30)
parser.add_argument('--immune_r', type=int, default=16, help="the radius of trust region")
...
def get_bias(shape):
    ...
    bias = Variable(bias.type(dtype), requires_grad=True)
    bias = bias.to(device)
    return bias
...
# Layer1
alpha=1.0
w1, force_layer1 = get_layer([28*28, 2000], alpha=alpha, args=args)
b1 = get_bias(w1.size(1))
# Layer2
w2, force_layer2 = get_layer([2000, 2000], alpha=alpha, args=args)
b2 = get_bias(w2.size(1))
# Layer_out
w3, force_layer3 = get_layer([2000, class_num], alpha=alpha, args=args)
...
for task_index in range(Task_num):
    ...
    for epoch in range(num_epochs):
        if task_index < num_epochs - immune_r and epoch <= num_epochs - immune_r:
            learning_rate = 0.10
        else:
            learning_rate = 2.0
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            force_layer1.force_learn(w1, input, learning_rate, alpha=alpha_array[0], flag=flag)
            force_layer2.force_learn(w2, output1, learning_rate, alpha=alpha_array[1], flag=flag)
            force_layer3.force_learn(w3, output2, learning_rate, alpha=alpha_array[2], flag=flag)
