...
class OWMLayer_cdp:
    ...
    def __init__(self, shape_list, alpha, l2_reg_lambda, train_context, contextual_information=1, LCDP=True, args=None):
        """
    ...
    def owm_learn(self, batch_x, batch_y, train=False, alpha_list=None, task_index=None, iowm=True, encoder_lr=10.0):
        ...
        if train:
            ...
            if self.train_context:
                self.P_c = self.learning(context_input, self.P_c, self.w_c, alpha_list[0]*encoder_lr, alpha_list[1], iowm=iowm)
            self.P1 = self.learning(batch_x, self.P1, self.w1, alpha_list[0], alpha_list[2], iowm=iowm)
        else:
    ...
    def learning(self, input_, Pro, weight, lr=0, alpha=1.0, iowm=True):
