...
class Appr(object):
    ...
    def __init__(self, model, sbatch=40, lr=0.2, input_size=784, hidden=800):
        """
        :param model:
        :param sbatch: batch_size
        :param lr: learning rate
        :param input_size: MNIST dataset width*height
        ...
        with torch.no_grad():
            ...
            self.P2 = torch.autograd.Variable(torch.eye(hidden + 1).type(dtype))
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid, data, lr, num_epochs=20, nums=0):
        """

        :param t: 当前执行的任务序号
        :param xtrain: 训练数据集
        :param ytrain: 训练数据集的标签
        :param xvalid: 测试集数据集
        :param yvalid: 测试集数据集的标签
        :param data: 所需测试的所有数据
        :param lr: 学习率
        :param num_epochs: 当前任务的迭代次数
        :param nums: 更新投影矩阵P的位置
        :return:
        """
        ...
        current_step = 0
        all_data = len(ytrain)
        all_step = all_data * self.nepochs // self.sbatch  # 在给定的batch和迭代次数下，当前数据集所需的迭代总次数
        # all_step = (all_data // self.sbatch) * self.nepochs
    ...
    def train_epoch(self, x, y, current_step=0, all_step=0, flag=True):
        ...
        for i_batch in range(0, len(r_len), self.sbatch):
            ...
            # alpha_array = [0.9 * 0.001 ** lamda, 0.5]
