...
def save_dataset(args):
    ...
    for p_ind, patient in enumerate(patients_list):
        # patient_input_path = os.path.join(args.data_path, patient, "input")
        patient_target_path = os.path.join(args.data_path, patient, "target")
        for path_ in [patient_target_path]:
            full_pixels = get_pixels_hu(load_scan(path_))
            for pi in range(len(full_pixels)):
                io = 'input' if 'input' in path_ else 'target'
                f = normalize(full_pixels[pi], args.norm_range_min, args.norm_range_max)
                f_name = '{}_{}_{}.raw'.format(patient, pi, io)
                f.tofile(os.path.join(args.save_path, f_name))
...
if __name__ == "__main__":
    ...
    # dir = "G:/add_noise_red_cnn/"
    dir = "./data/"
    dir_path = "./data/Mayo_abdomen/"
    parser.add_argument('--data_path', type=str, default=dir+'Mayo_abdomen/')
    parser.add_argument('--save_path', type=str, default=dir_path+'npy_img_Poisson_noise_1e4/')
